...
from functions.write_in_md_and_db import (
    ...
    write_activity_record_evaluation_in_db)



def get_evaluation_columns(db_path, table_name):
...
class Ui_activity_temp_closure_activity(object):  # from reminder_finalversion.py
    ...
    def __init__(self,Activity_Temp_Closure_Window):
        ...
        # 生成新的活动评价属性list
        self.current_activity_record.evaluation = get_evaluation_columns("D:\LAE_PC\LAE.db", 'activity_evaluation')
        ...
        # 将控件添加到右上布局
        self.right_top_layout.addWidget(self.activity_record_attribute_list)
        
        # 调用函数添加右下控件
        self.add_right_bottom_controls()
        
        self.retranslateUi(Activity_Temp_Closure_Window)
        QMetaObject.connectSlotsByName(Activity_Temp_Closure_Window)
    
    def add_right_bottom_controls(self):
        self.attribute_labels = {}
        self.attribute_value_labels = {}
        self.attribute_sliders = {}

        for i, attribute in enumerate(self.current_activity_record.evaluation):
            # 创建标签
            label = QLabel(self.centralwidget)
            label.setText(attribute.capitalize())
            self.right_bottom_layout.addWidget(label)
            self.attribute_labels[attribute] = label

            # 创建值标签
            value_label = QLabel(self.centralwidget)
            value_label.setText("0.0")
            self.right_bottom_layout.addWidget(value_label)
            self.attribute_value_labels[attribute] = value_label

            # 创建滑动条
            slider = QSlider(Qt.Horizontal, self.centralwidget)
            slider.setMinimum(10)
            slider.setMaximum(50)
            slider.setValue(30)
            slider.valueChanged.connect(lambda value, attr=attribute: self.update_attribute(attr, value))
            self.right_bottom_layout.addWidget(slider)
            self.attribute_sliders[attribute] = slider
    
    def update_attribute(self, attribute, value):
        activeness = value / 10.0
        self.attribute_value_labels[attribute].setText(str(activeness))
    ...
    def setupUi(self):
        ...
        self.activity_record_attribute_value_mapping ={
            ...
            "备注": self.current_activity_record.notes
        }
    ...
    def on_generate_activity_record_button_clicked(self):
        ...
        # 更新 evaluation 的值
        self.current_activity_record.evaluation = [
            {attribute: label.text()} for attribute, label in self.attribute_value_labels.items()
        ]
        write_activity_record_evaluation_in_db(self.current_activity_record)
