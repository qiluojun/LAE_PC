...
from functions.write_in_md_and_db import write_activity_record_in_db,find_path,write_single_activity_record_in_md



class Ui_new_activity_record(object):  # from reminder_finalversion.py
    ...
    def __init__(self,NewActivityWindow):
        ...
        self.delete_chosen_button = QPushButton("删除选中项属性", self.centralwidget)
        self.add_activity_record_button = QPushButton("添加活动记录", self.centralwidget)
        ...
        two_buttons_layout.addWidget(self.delete_chosen_button)

        two_buttons_layout.addWidget(self.add_activity_record_button)
        ...
        self.right_layout.addLayout(two_buttons_layout)
    ...
    def setupUi(self,db):
        ...
        # 处理 list（右下的活动属性列表）
        self.activity_activity_attributes = ["活动名称", "开始时间",  "结束时间", "实际时长", "实际内容","备注"]
        # 将属性列表的内容添加到activity_attribute_list中
        self.activity_attribute_list.addItems(self.activity_activity_attributes)
        ...
        # 连接 添加活动 按钮 
        self.add_activity_record_button.clicked.connect(self.activity_record_generate)        
        self.add_activity_record_button.clicked.connect(lambda: write_activity_record_in_db(self.current_activity_record))
        self.add_activity_record_button.clicked.connect(lambda: write_single_activity_record_in_md(self.current_activity_record))

    def activity_record_generate(self): 
        self.current_activity_record.record_timestamp=self.current_activity_record.generate_timestamp()
        self.current_activity_record.activity_status['status']='单次活动'
        self.current_activity_record.activity_status['times_performed']='1'
        # 顺便加一个 点以后 有√特效        
        original_text = self.add_activity_record_button.text()
        self.add_activity_record_button.setText("√")
        # Use QTimer.singleShot to change the text back to the original text after 1 second
        QTimer.singleShot(1000, lambda: self.add_activity_record_button.setText(original_text))
    ...
    def on_content_yes_button_clicked(self):
        ...
        self.current_activity_record.content_location = name
        # 添加新的列表项
    ...
    def on_type_yes_button_clicked(self):
        ...
        name = record.value("name")
        # 将name列的值赋给self.current_activity.name
        self.current_activity_record.type_location = name
        # 添加新的列表项
    ...
    def on_add_into_activity_button_clicked(self):
        ...
        if selected_attribute in ["开始时间", "结束时间","实际时长"]:
            self.current_activity_record.actual_duration[attribute_name_mapping[selected_attribute]] = text
        else:
            setattr(self.current_activity_record, attribute_name_mapping[selected_attribute], text)        
        # 添加新的列表项
    ...
    def on_delete_chosen_button_clicked(self):
        ...
        if selected_item is not None:
            ...
            if attribute_name in ["开始时间", "结束时间","实际时长"]:
                self.current_activity_record.actual_duration[attribute_name_mapping[attribute_name]] = ""
            else:
                setattr(self.current_activity_record, attribute_name_mapping[attribute_name], "")        
        
            #setattr(self.current_activity_record, attribute_name_mapping[attribute_name], "")


    def reset_current_activity_record(self):
        self.current_activity=Activity(name=" ")
        self.current_activity_record=ActivityRecord(self.current_activity)
    ...
    def retranslateUi(self,  NewActivityWindow):
        ...
        # 窗口名称
        NewActivityWindow.setWindowTitle(_translate("添加新活动记录", "TimeLineV2"))
...
if __name__ == "__main__":
    ...
    db.setDatabaseName("D:\\LAE_PC\\LAE.db")

    app = QApplication(sys.argv)
    NewActivityWindow = QMainWindow()
    ui = Ui_new_activity_record(NewActivityWindow)
    ui.setupUi(db)
