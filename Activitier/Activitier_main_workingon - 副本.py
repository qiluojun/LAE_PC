...
class new_activity_window(QMainWindow):
 ...
 # 初始化界面
    ...
    def closeEvent(self, event):
        ...
        if self.ui.db and self.ui.db.isOpen():
            self.ui.db.close()
            print("13")
...
class MySysTrayWidget(QWidget):
    def __init__(self, ui=None, app=None, window=None):
        ...
        self.__trayicon.setToolTip('TimeLine\n热键Ctrl+Alt+M')

        # 创建托盘的右键菜单
        self.__traymenu = QMenu()
        self.__trayaction = []
        self.addTrayMenuAction('显示TimeLine主界面', self.show_userinterface)
...
def open_start_new_activity_window():
    ...
    main_window.start_activity_window.ui.start_chosen_activity_button.clicked.connect(
        lambda: (
            ...
            print("main_window.new_activity_window.ui.current_activity.timestamp:",main_window.start_activity_window.ui.current_activity.timestamp),
...
def new_activity_start():
    ...
    if main_window.current_activity is None:
        print("Error: Failed to get current activity1.")
        return
    
    # 首先 更新获得的 活动的 各项内容 
    print("main_window.current_activity.name:",main_window.current_activity.name)
    main_window.current_activity = get_activity(main_window.current_activity.timestamp)
    if main_window.current_activity is None:
        print("Error: Failed to get current activity2.")
        return
    # 更新活动标签
...
def start_activity_timer():
    ...
    # 开始计时器每秒更新标签
    activity_timer.start(4000)  # 每1s更新一次
    continuous_timer.start(4000)  # 连续计时器也每1s更新一次
...
def update_label():
    ...
    if not is_paused:
        # 计算经过的时间
        activity_length += timedelta(seconds=4)
        total_seconds = activity_length.total_seconds()
...
def update_continuous_label():
    ...
    if not is_paused:
        # 计算经过的时间
        continuous_activity_length += timedelta(seconds=4)
        total_seconds = continuous_activity_length.total_seconds()
        hours = int(total_seconds // 3600)
        minutes = int((total_seconds % 3600) // 60)
        seconds = int(total_seconds % 60)
