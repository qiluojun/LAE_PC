...
def start_activity_timer():
    ...
    # 开始计时器每秒更新标签
    activity_timer.start(5000)  # 每5s更新一次
    continuous_timer.start(5000)  # 连续计时器也每5s更新一次
...
def update_label():
    ...
    if not is_paused:
        # 计算经过的时间
        activity_length += timedelta(seconds=5)
        total_seconds = activity_length.total_seconds()
...
def update_continuous_label():
    ...
    if not is_paused:
        # 计算经过的时间
        continuous_activity_length += timedelta(seconds=5)
        total_seconds = continuous_activity_length.total_seconds()
        hours = int(total_seconds // 3600)
        minutes = int((total_seconds % 3600) // 60)
        seconds = int(total_seconds % 60)
