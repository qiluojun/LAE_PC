...
class new_activity_window(QMainWindow):
 ...
 # 使用ui文件，导入定义的界面类
        ...
        self.ui.connect_signals_and_slots()
 # 初始化界面
        #self.ui.setupUi(db)
...
class start_new_activity_window(QMainWindow):
 ...
 # 初始化界面
    ...
    def closeEvent(self, event):
        if self.ui.db and self.ui.db.isOpen():
            self.ui.db.close()
        print("start_new_activity_window is closed")
        event.accept()
...
def open_start_new_activity_window():
    ...
    main_window.start_activity_window.ui.setupUi(db)
...
main_window.start_activity_window.ui.add_new_activity_button.clicked.connect(
    lambda: (
        main_window.new_activity_window.ui.setupUi(db),
