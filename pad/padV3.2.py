...
side_line = data.get("side_line", [
    ...
    {"name": "Sideline 6", "location": "Location 6", "date": "Date 6"},
    {"name": "Sideline 7", "location": "Location 7", "date": "Date 7"},
    {"name": "Sideline 8", "location": "Location 8", "date": "Date 8"},
])
...
sideline_scrollbar.grid(row=3, column=2, rowspan=4, sticky=tk.N+tk.S)
sideline_text_widget = tk.Text(right_frame, wrap=tk.WORD, yscrollcommand=sideline_scrollbar.set, width=50, height=20, font=("TkDefaultFont", 16), fg="red")
sideline_text_widget.grid(row=3, column=0, rowspan=4, padx=10, pady=10, sticky="w")

# Add sideline names and details to the Text widget
for sideline in side_line:
...
# Load the value of rules from a json file
with open("data2.json", "r") as f:
    data = json.load(f)
    if "rules" in data:
        rules_value = data["rules"]
    else:
        rules_value = "" # or any default value you want to assign

# Assign the value of rules
rules = tk.StringVar(value=rules_value)

# Create a message widget to display the content of "rules"
rules_message = tk.Message(left_frame, textvariable=rules, font=("TkDefaultFont", 12), width=400)
rules_message.grid(row=6, column=0, padx=10, pady=10, sticky="w")

# Create a function to update the content of "rules"
def update_rules():
    # Create a child window
    child_window = tk.Toplevel(root)
    child_window.title("修改规则~")

    # Create a message widget to prompt the user for input
    rules_label = tk.Message(child_window, text="请输入新的内容：")
    rules_label.grid(row=0, column=0, padx=10, pady=10)

    # Create a text widget to allow the user to enter the new content of "rules"
    rules_text = tk.Text(child_window, height=10, width=50)
    rules_text.grid(row=1, column=0, padx=10, pady=10)

    # Create a function to update the content of "rules"
    def update():
        rules.set(rules_text.get("1.0", "end-1c"))
        child_window.destroy()

        # Save the value of rules to a json file
        with open("data2.json", "w") as f:
            data["rules"] = rules.get()
            json.dump(data, f)

    # Create an "OK" button to update the content of "rules"
    ok_button = tk.Button(child_window, text="OK", command=lambda: update())
    ok_button.grid(row=2, column=0, padx=10, pady=10)

# Create a button to call the update_rules function
update_rules_button = tk.Button(left_frame, text="修改规则~", command=update_rules)
update_rules_button.grid(row=7, column=0, padx=10, pady=10, columnspan=2, sticky="w")
