...
'''
V3.1 更新内容： gui换库咯~ 可喜可贺！
更新思路：
1. 思考 lock screen 如何触发 
要新创建一个字典库 装对应时段触发的对应提示内容 
要更新lock_screen 内容  时段指标 可以从主py传过来 也可以由界面自定义修改 然后刷新
    这一条模仿之前的东东就可以 
2. 关于字典库 即提示信息库……嗯……思考ing…… 先考虑时段通用tips 有了 timeline.md的“试试”可用 或许



优化思路：locking screen 里的提示语可以随机选一条哈哈 不一定一直是一样的 

未来更新思路：
做到 电脑休眠中唤醒 程序check类function也会被触发 而不仅是每隔固定时段进行触发
'''
...
'''
def lock():
    #main_window.lock_window= LockScreenWindow(0.1,period_index1)
...
def basic_state_record():
    ...
    def basic_state_record_generate():
        ...
        basic_state['tiredness'] = (main_window.child_window4.ui.tiredness_entry.text())
        basic_state['exhaustion'] = (main_window.child_window4.ui.exhaustion_entry.text())
        basic_state['anxiety'] = (main_window.child_window4.ui.anxiety_entry.text())
        basic_state['vitality'] = (main_window.child_window4.ui.vitality_entry.text())
        basic_state['activeness'] = (main_window.child_window4.ui.activeness_entry.text())
        content=''
