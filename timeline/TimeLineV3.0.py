...
增加 【基本状态】简单记录功能
    包括:
        困倦度tiredness 脑损度exhaustion  体力值vitality 
        焦虑度anxiety 积极愉悦度activeness 时间time
功能实现： 
    点击timeline主窗口对应按钮
    子窗口[basic state recorder]中填写对应内容
    点击 确定
    窗口自动消失
    数据自动写入md文件 
V3.1 更新预想：
1. 增加新的 单独的程序 将md文件 转换成excel？或者考虑其他的数据处理方式 
2. 增加其他部分的参数记录——待定
'''
...
def basic_state_record():
    child_window = tk.Toplevel()
    child_window.title("记录基础状态")
    child_window.attributes('-topmost', True)
    child_window.geometry("800x400")
    # 创建属性输入框
    tiredness_label = tk.Label(child_window, text="困倦度：", font=("TkDefaultFont", 20), fg="blue")
    tiredness_label.grid(row=0, column=0, padx=10, pady=10, sticky="w")
    tiredness_entry = tk.Entry(child_window, font=("TkDefaultFont", 20))
    tiredness_entry.grid(row=0, column=1, padx=10, pady=10, sticky="w")

    exhaustion_label = tk.Label(child_window, text="脑损度：", font=("TkDefaultFont", 20), fg="blue")
    exhaustion_label.grid(row=1, column=0, padx=10, pady=10, sticky="w")
    exhaustion_entry = tk.Entry(child_window, font=("TkDefaultFont", 20))
    exhaustion_entry.grid(row=1, column=1, padx=10, pady=10, sticky="w")

    anxiety_label = tk.Label(child_window, text="焦虑度：", font=("TkDefaultFont", 20), fg="blue")
    anxiety_label.grid(row=2, column=0, padx=10, pady=10, sticky="w")
    anxiety_entry = tk.Entry(child_window, font=("TkDefaultFont", 20))
    anxiety_entry.grid(row=2, column=1, padx=10, pady=10, sticky="w")

    vitality_label = tk.Label(child_window, text="体力值：", font=("TkDefaultFont", 20), fg="blue")
    vitality_label.grid(row=3, column=0, padx=10, pady=10, sticky="w")
    vitality_entry = tk.Entry(child_window, font=("TkDefaultFont", 20))
    vitality_entry.grid(row=3, column=1, padx=10, pady=10, sticky="w")

    activeness_label = tk.Label(child_window, text="积极愉悦度：", font=("TkDefaultFont", 20), fg="blue")
    activeness_label.grid(row=4, column=0, padx=10, pady=10, sticky="w")
    activeness_entry = tk.Entry(child_window, font=("TkDefaultFont", 20))
    activeness_entry.grid(row=4, column=1, padx=10, pady=10, sticky="w")

    def basic_state_record_generate():
        basic_state={}
        basic_state['time']= datetime.now().strftime("%D:%H:%M")
        basic_state['tiredness'] = (tiredness_entry.get())
        basic_state['exhaustion'] = (exhaustion_entry.get())
        basic_state['anxiety'] = (anxiety_entry.get())
        basic_state['vitality'] = (vitality_entry.get())
        basic_state['activeness'] = (activeness_entry.get())
        content=''
        ...
        with open(r'D:\学习and学校\obsidian\qiluo\00-MOC\basic_state.md','a',encoding='utf-8') as f:
            f.write(content)
        # 创建确定按钮
    yes_button = tk.Button(child_window, text="生成记录", command=basic_state_record_generate, font=("TkDefaultFont", 20))
    yes_button.grid(row=5, column=0, padx=10, pady=10, sticky="w")
...
#  创建 【基本状态记录】 按钮
update_button = tk.Button(left_frame, text="记录基本状态", command=basic_state_record, font=("TkDefaultFont", 20))
