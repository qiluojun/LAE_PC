...
# 手动更新plan
button = tk.Button(left_frame, text='Show plan', command= read_plan,font=("TkDefaultFont",16))
...
# 下面是呈现activity的部分
activity_listbox = tk.Listbox(right_frame, font=("TkDefaultFont", 20))
...
def end_activity():
    ...
    if current_activity:
        ...
        # 先定义出要写的 content
        content = '\n\n{}-{}  {} vs {}\n{}'.format(current_activity['start_time'],current_activity['end_time'],current_activity['lenth'],current_activity['time'],current_activity['content'])
        # 写入
...
end_activity_button.grid(row=7, column=1, padx=10, pady=10, sticky="w")
# 主界面 呈现当前活动
