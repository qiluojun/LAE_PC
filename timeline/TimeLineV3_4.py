...
'''
3.4 更新内容 新时段开始自动锁屏改为弹窗 并且显示内容由reminder.db储存

进度：
    俺在桌面对应excel中思考了一哈reminder大概在db里涉及的属性等 编程还没来得及编和改
    抓手 算法差不多后 先把自动锁屏删掉 改成弹窗 
...
def check_period():
    ...
    if (current_time >= '2140' and current_time <='2200') or current_time >= '2300'  or current_time == '1216':
        lock(0.2) # 锁屏时长 #ceshi 
